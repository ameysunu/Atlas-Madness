...
//    var formattedString = jsonString
//
//    formattedString = formattedString.trimmingCharacters(in: .whitespacesAndNewlines)
//    formattedString = "[" + formattedString.dropFirst().dropLast() + "]"
//
//    // Replace colons after keys with colons inside double quotes
//    formattedString = formattedString.replacingOccurrences(of: "([a-zA-Z0-9_]+):", with: "\"$1\":", options: .regularExpression)
//
//    // Add double quotes around values that are not already quoted
//    formattedString = formattedString.replacingOccurrences(of: "(\"(.*?)\"|[_a-zA-Z0-9.-]+(?!\\s*:\\s*\"))", with: "\"$2\"", options: .regularExpression)
//
//
...
func formatJSONString(_ jsonString: String) -> String {
    ...
    formattedString = String(formattedString.dropFirst().dropLast())
    
    // Replace colons after keys with colons inside double quotes
    formattedString = formattedString.replacingOccurrences(of: "\\s*:\\s*", with: ":", options: .regularExpression)
    
    // Add double quotes around unquoted keys
    formattedString = formattedString.replacingOccurrences(of: "(\\w+):", with: "\"$1\":", options: .regularExpression)
    
    // Add commas between key-value pairs
    formattedString = formattedString.replacingOccurrences(of: ",\\s*(\\}|\\])", with: "$1", options: .regularExpression)
    
    // Enclose the entire JSON content with square brackets
