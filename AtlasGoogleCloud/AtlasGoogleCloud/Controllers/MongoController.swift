...
func addMoodData(userid: String, mood: String, rating: Double, notes: String, context: String, trigger: String, sleepQuality: Double, appetite: Double, energyLevel: Double, completion: @escaping (String) -> Void) {
    guard let url = URL(string: "https://eu-west-1.aws.data.mongodb-api.com/app/data-wnfuh/endpoint/data/v1/action/insertOne") else {
        print("Invalid URL")
        return
    }
    ...
    let parameters = [
        ...
        "database": "users",
        "dataSource": "MyCluster",
        "document": ["id": "\(userid)", "mood": "\(mood)", "rating": "\(rating)", "notes": "\(notes)", "context": "\(context)", "trigger": "\(trigger)", "sleepQuality": "\(sleepQuality)", "appetite": "\(appetite)", "energyLevel": "\(energyLevel)"]
    ] as [String : Any]
